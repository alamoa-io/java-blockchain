...
public class Wallet {
    ...
    private String generateBlockchainAddress(PublicKey publicKey) throws Exception {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(1 + ripemd160Hash.length); // 1バイト (networkByte) + 20バイト (RIPEMD-160ハッシュ)
        ...
        ByteBuffer finalAddressBuffer = ByteBuffer.allocate(networkBitcoinPublicKeyBytes.length + checksumBytes.length);
